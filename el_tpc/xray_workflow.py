...
def _check_excessive_sN(v_window: np.ndarray, dt: float, max_area: float, bs_threshold: float) -> np.ndarray:
    """Returns boolean mask of frames passing the S1 area check."""
    v_thresh = np.where(v_window > bs_threshold, v_window, 0)   # Mask out values below threshold
    areas = np.trapezoid(v_thresh, dx=dt, axis=1)               # Integrate over time to get area
    return areas <= max_area

def _check_signal_separation(v_win: np.ndarray, t_win: np.ndarray, 
                             t_s1: float, s2_start: float, 
                             min_s1_sep: float, min_s2_sep: float) -> np.ndarray:
    """Returns boolean mask of frames where the X-Ray is safely separated from S1 and S2."""
    sig_mask = v_win > 0
    has_signal = sig_mask.any(axis=1)

    # Argmax finds the FIRST True value (left edge). Reversing it finds the LAST (right edge).
    first_idx = np.argmax(sig_mask, axis=1)
    last_idx = v_win.shape[1] - 1 - np.argmax(sig_mask[:, ::-1], axis=1)

    t_first = t_win[first_idx]
    t_last = t_win[last_idx]

    return ((t_first - t_s1) > min_s1_sep) & \
           ((s2_start - t_last) > min_s2_sep) & \
           has_signal

def _update_stats(stats: dict, pass_s1: np.ndarray, pass_s2: np.ndarray, pass_sep: np.ndarray, pass_area: np.ndarray):
    """Helper to update the stats dictionary with the results from a batch."""
    stats['total'] += len(pass_s1)
    stats['pass_s1'] += int(pass_s1.sum())
    stats['pass_s2'] += int(pass_s2.sum())
    stats['pass_sep'] += int(pass_sep.sum())
    stats['pass_area'] += int(pass_area.sum())
    stats['accepted'] += int((pass_s1 & pass_s2 & pass_sep & pass_area).sum())

def _print_xray_stats(stats: dict, n_acc: int):
    tot = stats['total']
    print(f"    ✓ Classified {tot} frames: {n_acc} accepted ({(n_acc/tot)*100:.1f}%), {tot - n_acc} rejected")
    print(f"      - Excessive S1 reject:  {tot - stats['pass_s1']}")
    print(f"      - Excessive S2 reject:  {tot - stats['pass_s2']}")
    print(f"      - Poor Separation:      {tot - stats['pass_sep']}")
    print(f"      - Below Min Area:       {tot - stats['pass_area']}")
...
def _aggregate_run_stats(run: Run, json_stats: list):
    ...
    agg_stats = {'total': 0, 'pass_s1': 0, 'pass_s2': 0, 'pass_sep': 0, 'pass_area': 0, 'accepted': 0}
...
def calculate_xray_areas(set_pmt: SetPmt, 
    ...
    """Single-pass 2D vectorized classification and integration of X-Rays."""
    
    t_s1 = float(set_pmt.t_s1)
    s2_start = float(set_pmt.t_s2_start) if set_pmt.t_s2_start else t_s1 + float(set_pmt.time_drift)
    print(f"  Searching X-Ray window: [{t_s1:.2f}, {s2_start:.2f}] µs")

    out_uids, out_areas = [], []
    stats = {'total': 0, 'pass_s1': 0, 'pass_s2': 0, 'pass_sep': 0, 'pass_area': 0, 'accepted': 0}

    for wf in file_ops.iter_waveforms(set_pmt, max_files=max_files):
        ...
        v_s1 = v_2d[:, wf.t < t_s1] # Slice for S1 check
        v_s2 = v_2d[:, wf.t >= s2_start] # Slice for S2 check
        win_mask = (wf.t >= t_s1) & (wf.t < s2_start) # Mask for the X-Ray window (used in separation check and area calculation)
        
        # 2. Execute Vectorized Checks
        pass_s1 = _check_excessive_sN(v_s1, config.dt, config.max_area_s1, config.bs_threshold)
        pass_s2 = _check_excessive_sN(v_s2, config.dt, config.max_area_s2, config.bs_threshold)
        pass_sep = _check_signal_separation(v_2d[:, win_mask], wf.t[win_mask], t_s1, s2_start, 
                                            config.min_s1_sep, config.min_s2_sep)

        # 3. Integrate areas on the drift window (X-ray candidates) 
        areas = np.trapezoid(v_2d[:, win_mask], dx=config.dt, axis=1)
        pass_area = areas >= config.min_xray_area

        # 4. Filter and Store
        final_mask = pass_s1 & pass_s2 & pass_sep & pass_area
        out_uids.append(wf.uids[final_mask])
        out_areas.append(areas[final_mask])

        # 5. Log Chunk Stats
        _update_stats(stats, pass_s1, pass_s2, pass_sep, pass_area)
    ...
    n_acc = len(final_uids)
    _print_xray_stats(stats, n_acc)
