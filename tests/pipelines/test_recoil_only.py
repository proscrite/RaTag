...
def test_recoil_pipeline_end_to_end(test_run, tmp_path):
    ...
    # Full preparation
    run = prepare_run(test_run, max_frames=2, max_frames_s1=20, max_frames_s2=50)
    
    # Run recoil analysis
    run = recoil_pipeline(run, max_frames=2)
    
    # Check that sets have S2 area results in metadata
    sets_with_s2 = [s for s in run.sets if s.area_s2_mean is not None]
...
def test_recoil_pipeline_subset(test_run):
    ...
    run = recoil_pipeline(run, range_sets=slice(0, 2), max_frames=2)
...
def test_recoil_pipeline_replot(test_run):
    ...
    n_results_first = len([s for s in run.sets if s.area_s2_mean is not None])
    ...
    n_results_second = len([s for s in run_replotted.sets if s.area_s2_mean is not None])
...
def test_pipeline_uses_caching(test_run):
    ...
    # First run
    run1 = recoil_pipeline(run, max_frames=2)
    
    # Second run should use cache (metadata already exists)
    run2 = recoil_pipeline(run1, max_frames=2)
    
    # Results should be identical (check metadata)
    for s1, s2 in zip(run1.sets, run2.sets):
        if s1.area_s2_mean is not None and s2.area_s2_mean is not None:
            assert s1.area_s2_mean == s2.area_s2_mean
            assert s1.area_s2_ci95 == s2.area_s2_ci95
