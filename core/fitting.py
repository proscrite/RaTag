...
def _fit_signal_crystalball(cbins: np.ndarray,
    ...
    # Ensure inputs are 1-D numpy arrays and shapes match
    cbins = np.asarray(cbins).ravel()
    n = np.asarray(n).ravel()
    if cbins.shape != n.shape:
        raise ValueError(f"cbins and counts must have the same shape: cbins={cbins.shape}, counts={n.shape}")

    if len(cbins) == 0:
        raise ValueError("No data provided to _fit_signal_crystalball (empty arrays)")
    ...
    params_sig['sig_sigma'].set(min=0.1, max=2.0)
    ...
    try:
        ...
        # Re-raise with diagnostic information to help debugging shape/mapping issues
        raise RuntimeError(
            f"_fit_signal_crystalball: signal fit failed: {e}. "
            f"cbins.shape={cbins.shape}, counts.shape={n.shape}, "
            f"cbins[:5]={cbins[:5].tolist()}, counts[:5]={n[:5].tolist()}"
        ) from e
