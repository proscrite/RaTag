...
def execute_gamma_pipeline(config_path: str, output_dir: str = "artifacts/gamma_spectroscopy"):
    ...
    # Plot accumulation graphs (rate count and activities)
    plot_accumulation_activities(accumulation_metrics, out_path )
    plot_rate_counts(accumulation_metrics=accumulation_metrics, out_path=out_path)
    ...
    # 6. Combine with Alpha data
    print("Merging with Th-228 alpha activity...")
    desorption_metrics = compute_desorption_probabilities(metrics = accumulation_metrics, 
                                                          rate_batches = rate_batches,
                                                          mca_csv_path = config['mca_csv_path'], 
                                                          mca_channels = config['mca_channels'],
                                                          foil_geometry_fraction = config['foil_geometry_fraction'])
    
    # 7. Final Export
    export_accumulation_summary(desorption_metrics, out_path / "accumulation_summary.csv")
